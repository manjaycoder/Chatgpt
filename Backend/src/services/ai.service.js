...
async function generateVector(content) {
    try {
        if (!content || typeof content !== "string" || content.trim().length === 0) {
            throw new Error("Content must be a non-empty string for embedding");
        }
        ...
        const response = await ai.models.embedContent({
            ...
            config: { outputDimensionality: 1024 } // Ensure the output dimensionality matches the Pinecone index
        });

        const vector = response?.embeddings[0]?.values;

        if (!vector || vector.length !== 1024) {
            throw new Error("Generated vector does not match the required dimensionality of 1024");
        }
